...
// rawsource.h
#ifndef _SJASM_RAWSOURCE_
#define _SJASM_RAWSOURCE_
...
public:
	RSStructure(int n_offset, int n_align, bool n_global, Structure &n_s, StringList &sl) : 
		RawSource(sl),
		_offset(n_offset), _newalign(0), _defaultalign(n_align), 
		_global(n_global), _mustalign(false), 
		_s(n_s) {
	}
private:
	void _Parse(string&);
