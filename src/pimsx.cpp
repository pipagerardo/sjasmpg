...
void getOperand(string &ns, Op &op) {
	...
	if( s[0]=='[' ) {
		op.ind=2; blokhaakje=1; s.erase(0,1);
	} else if( !options.onlybp && s[0]=='(' ) {
		op.ind=1; blokhaakje=0; hs=s; s.erase(0,1);
	}
	op.reg=getz80reg(s);
	if( op.ind &&
	...
	if (op.ind) {
		if (blokhaakje) {
			if (!sbcneed(s,']')) error(EM_Closing_C_expected);
		} else {
			if (op.reg!=Z80_nn && !sbcneed(s,')')) error(EM_Closing_P_expected);
...
int getjumpoperand(string &ns, Op &op) {
	...
	if (!res) {
		if (pass==1) labsnok=1;
		else error(EM_Label_not_found,ns);
	}
	return 1;
}
...
void NegateCondition( Z80Reg &x ) {
	switch (x) {
		...
		default: error(EM_Cannot_negate_condition);
	}
}
...
int undoc( string &p, int r ) {
	...
	switch( int reg=getz80reg(p) ) {
		...
		default: error(EM_Illegal_operand); break;
	}
	return r+6;
}
	...
	switch (x.reg) {
		case Z80_A:
			switch (y.reg) {
				...
				case Z80mnn:
					if (y.ind>1) { error(EM_Indirection_not_allowed); break; }
				case Z80_nn: e[0]=0xce; e[1]=check8(y.val); break;
				case Z80mHL: case Z80miHL: case Z80mdHL: case Z80mHLi: case Z80mHLd:
					preinc(e,y); e.push(0x8e); postinc(e,y);
				break;
				case Z80mIX: case Z80miIX: case Z80mdIX: case Z80mIXi: case Z80mIXd:
					preinc(e,y); e.push(0xdd); e.push(0x8e);
					e.push(checki(y.val)); postinc(e,y);
					break;
				case Z80mIY: case Z80miIY: case Z80mdIY: case Z80mIYi: case Z80mIYd:
					preinc(e,y); e.push(0xfd); e.push(0x8e);
					e.push(checki(y.val)); postinc(e,y);
				break;
				default: error(EM_Illegal_operand,y.str,ERRREP); break;
			}
		break;
		case Z80_HL:
			switch (y.reg) {
				case Z80_BC: case Z80_DE: case Z80_HL: case Z80_SP:
					e[0]=0xed; e[1]=y.reg+0x3a;
				break;
				default: error(EM_Illegal_operand,y.str,ERRREP); break;
			}
		break;
		default:
			error(EM_Illegal_operand,x.str,ERRREP);
		break;
	}
	checkjunk(line);
}
	...
	switch (x.reg) {
		case Z80_A:
			switch (y.reg) {
				...
				case Z80mHL: case Z80miHL: case Z80mdHL: case Z80mHLi: case Z80mHLd:
					preinc(e,y); e.push(0x86); postinc(e,y);
				break;
				case Z80mIX: case Z80miIX: case Z80mdIX: case Z80mIXi: case Z80mIXd:
					preinc(e,y); e.push(0xdd); e.push(0x86);
					e.push(checki(y.val)); postinc(e,y);
				break;
				case Z80mIY: case Z80miIY: case Z80mdIY: case Z80mIYi: case Z80mIYd:
					preinc(e,y); e.push(0xfd); e.push(0x86);
					e.push(checki(y.val)); postinc(e,y);
				break;
				default: error(EM_Illegal_operand,ERRREP); break;
			}
		break;
		case Z80_HL:
			switch (y.reg) {
		...
		case Z80_IY:
			switch (y.reg) {
				...
				case Z80_BC: case Z80_DE: case Z80_SP:
					e[0]=0xfd; e[1]=y.reg-7;
				break;
				default: error(EM_Illegal_operand,ERRREP); break;
			}
		break;
		default:
			error(EM_Illegal_operand,ERRREP);
		break;
	}
	checkjunk(line);
}
	...
	switch (x.reg) {
		case Z80_A:
			switch (y.reg) {
				...
				case Z80mIX: case Z80miIX: case Z80mdIX: case Z80mIXi: case Z80mIXd:
					preinc(e,y); e.push(0xdd); e.push(0xa6);
					e.push(checki(y.val)); postinc(e,y);
				break;
				case Z80mIY: case Z80miIY: case Z80mdIY: case Z80mIYi: case Z80mIYd:
					preinc(e,y); e.push(0xfd); e.push(0xa6);
					e.push(checki(y.val)); postinc(e,y);
				break;
				default: error(EM_Illegal_operand,ERRREP); break;
			}
...
void pmBIT( string line, Data &e ) {
	...
	switch (x.reg) {
		case Z80mnn: if (x.ind>1) { error(EM_Indirection_not_allowed); break; }
		case Z80_nn:
			bit=x.val;
			if (bit<0 || bit>7) { error(EM_Illegal_operand,ERRREP); bit&=7; }
			switch (y.reg) {
				case Z80_B: case Z80_C: case Z80_D: case Z80_E:
				case Z80_H: case Z80_L: case Z80_A:
					e[0]=0xcb; e[1]=8*bit+0x40+y.reg; break;
				case Z80mHL: case Z80miHL: case Z80mdHL: case Z80mHLi: case Z80mHLd:
					preinc(e,y); e.push(0xcb);
					e.push(8*bit+0x46); postinc(e,y);
				break;
				case Z80mIX: case Z80miIX: case Z80mdIX: case Z80mIXi: case Z80mIXd:
					preinc(e,y); e.push(0xdd); e.push(0xcb);
					e.push(checki(y.val)); e.push(8*bit+0x46); postinc(e,y);
				break;
				case Z80mIY: case Z80miIY: case Z80mdIY: case Z80mIYi: case Z80mIYd:
					preinc(e,y); e.push(0xfd); e.push(0xcb);
					e.push(checki(y.val)); e.push(8*bit+0x46); postinc(e,y);
				break;
				default: error(EM_Illegal_operand,ERRREP); break;
			}
...
void pmCALL( string line, Data &e ) {
	...
	switch (y.reg) {
		case Z80mnn: if (y.ind>1) { error(EM_Indirection_not_allowed); break; }
		case Z80_nn:
	...
	switch (x.reg) {
		case Z80_A:
			switch (y.reg) {
				...
				case Z80mHL: case Z80miHL: case Z80mdHL: case Z80mHLi: case Z80mHLd:
					preinc(e,y); e.push(0xbe); postinc(e,y);
				break;
				case Z80mIX: case Z80miIX: case Z80mdIX: case Z80mIXi: case Z80mIXd:
					preinc(e,y); e.push(0xdd); e.push(0xbe);
					e.push(checki(y.val)); postinc(e,y);
				break;
				case Z80mIY: case Z80miIY: case Z80mdIY: case Z80mIYi: case Z80mIYd:
					preinc(e,y); e.push(0xfd); e.push(0xbe);
					e.push(checki(y.val)); postinc(e,y);
				break;
				default: error(EM_Illegal_operand,ERRREP); break;
			}
...
void pmDEC( string line, Data &e ) {
	...
	switch (x.reg) {
		...
		case Z80mIX: case Z80miIX: case Z80mdIX: case Z80mIXi: case Z80mIXd:
			preinc(e,x); e.push(0xdd); e.push(0x35);
			e.push(checki(x.val)); postinc(e,x);
		break;
		case Z80mIY: case Z80miIY: case Z80mdIY: case Z80mIYi: case Z80mIYd:
			preinc(e,x); e.push(0xfd); e.push(0x35);
			e.push(checki(x.val)); postinc(e,x);
		break;
...
void pmDJNZ( string line, Data &e ) {
	...
	while (!line.empty() && line[0]=='.') {
		...
		else error(EM_Unknown_instruction,"djnz."+p);
	}
	...
	switch (x.reg) {
		...
		case Z80_nn:
			...
			if (jmp<-128 || jmp>127) {
				if (opt) {
					e[0]=0x05; e[1]=0xc2; check16(x.val);
					e[2]=x.val&255; e[3]=x.val>>8;
				} else {
...
void pmEX( string line, Data &e ) {
	...
	switch (x.reg) {
		...
		break;
		case Z80_AF:
			if (no==2 && y.reg!=Z80_AF) { error(EM_Illegal_operand,ERRREP); break; }
			e[0]=0x08;
		break;
		case Z80_DE:
			if (y.reg!=Z80_HL) { error(EM_Illegal_operand,ERRREP); break; }
			e[0]=0xeb;
		break;
		default: error(EM_Illegal_operand,ERRREP); break;
	}
...
void pmIN( string line, Data &e ) {
	...
	switch (y.reg) {
		...
		break;
		case Z80mnn:
			if (x.reg!=Z80_A) { error(EM_Illegal_operand,ERRREP); break; }
...
void pmINC( string line, Data &e ) {
	...
	switch (x.reg) {
		...
		case Z80mIX: case Z80miIX: case Z80mdIX: case Z80mIXi: case Z80mIXd:
			preinc(e,x); e.push(0xdd); e.push(0x34);
			e.push(checki(x.val)); postinc(e,x);
		break;
		case Z80mIY: case Z80miIY: case Z80mdIY: case Z80mIYi: case Z80mIYd:
			preinc(e,x); e.push(0xfd); e.push(0x34);
			e.push(checki(x.val)); postinc(e,x);
		break;
...
void pmJP( string line, Data &e ) {
	...
	while (!line.empty() && line[0]=='.') {
		...
		else error(EM_Unknown_instruction,"jp."+p);
	}
	if (sbcneed(line,'!')) negcond=true;
	if (getJumpOperands2(line,x,y)==1) { y=x; x.reg=Z80_DUMMY; }
	switch (y.reg) {
		case Z80_HL: case Z80mHL:
			if (x.reg!=Z80_DUMMY) { error (EM_Illegal_operand); break; }
			e[0]=0xe9;
		break;
		case Z80_IX: case Z80mIX: case Z80_IY: case Z80mIY:
			if (x.reg!=Z80_DUMMY) { error (EM_Illegal_operand); break; }
			if (y.ind==3) { error( EM_Offsets_not_allowed_jumps ); break; }
	...
	if (c) {
		if(opt) {
            if (cr) {
                again=1;
                val=y.val-adres-2;
                if (val>-129 && val<128) { e[0]=cr; e[1]=val; goto pmJP_break; }
            }
		} else {
...
void pmJR( string line, Data &e ) {
	...
	while (!line.empty() && line[0]=='.') {
		...
		else error(EM_Unknown_instruction,"jr."+p);
	}
	if (sbcneed(line,'!')) negcond=true;
	if (getJumpOperands2(line,x,y)==1) { y=x; x.reg=Z80_DUMMY; }
	switch (y.reg) {
...
void pmLD( string line, Data &e ) {
	Op x,y;
	if (getOperands2(line,x,y)!=2) {
		error(EM_Illegal_operand,ERRREP); return;
	}
	switch (x.reg) {
		case Z80mBC: case Z80miBC: case Z80mdBC: case Z80mBCi: case Z80mBCd:
			if (y.reg!=Z80_A) break;
			preinc(e,x); e.push(0x02); postinc(e,x);
		break;
		case Z80mDE: case Z80miDE: case Z80mdDE: case Z80mDEi: case Z80mDEd:
			if (y.reg!=Z80_A) break;
			preinc(e,x); e.push(0x12); postinc(e,x);
		break;
		case Z80mHL: case Z80miHL: case Z80mdHL: case Z80mHLi: case Z80mHLd:
			switch (y.reg) {
				case Z80_A: case Z80_B: case Z80_C: case Z80_D:
				case Z80_E: case Z80_H: case Z80_L:
					preinc(e,x); e.push(0x70+y.reg); postinc(e,x);
				break;
				case Z80mnn:
					if (y.ind==2) { error(EM_Indirection_not_allowed); break; }
				case Z80_nn:
					preinc(e,x); e.push(0x36); e.push(check8(y.val)); postinc(e,x);
				break;
				default: break;
			}
		break;
		case Z80mIX: case Z80miIX: case Z80mdIX: case Z80mIXi: case Z80mIXd:
			switch (y.reg) {
				case Z80_A: case Z80_B: case Z80_C: case Z80_D:
				case Z80_E: case Z80_H: case Z80_L:
					preinc(e,x); e.push(0xdd); e.push(0x70+y.reg);
					e.push(checki(x.val)); postinc(e,x);
				break;
				case Z80mnn:
					if (y.ind==2) { error(EM_Indirection_not_allowed); break; }
				case Z80_nn:
					preinc(e,x); e.push(0xdd); e.push(0x36);
					e.push(checki(x.val)); e.push(check8(y.val)); postinc(e,x);
				break;
				default: break;
			}
		break;
		case Z80mIY: case Z80miIY: case Z80mdIY: case Z80mIYi: case Z80mIYd:
			switch (y.reg) {
				case Z80_A: case Z80_B: case Z80_C: case Z80_D:
				case Z80_E: case Z80_H: case Z80_L:
					preinc(e,x); e.push(0xfd); e.push(0x70+y.reg);
					e.push(checki(x.val)); postinc(e,x);
				break;
				case Z80mnn:
					if (y.ind==2) { error(EM_Indirection_not_allowed); break; }
				case Z80_nn:
					preinc(e,x); e.push(0xfd); e.push(0x36);
					e.push(checki(x.val)); e.push(check8(y.val)); postinc(e,x);
				break;
				default: break;
			}
		...
		case Z80mnn:
			switch (y.reg) {
				...
				case Z80_IX: case Z80_IY:
					e.push(y.reg); e.push(0x22);
				break;
				case Z80_BC: case Z80_DE: case Z80_SP:
					e.push(0xed); e.push(0x33+y.reg);
				break;
				default: break;
			}
			if (!e.size()) break;
		...
		case Z80_A:
			switch (y.reg) {
				case Z80mBC: case Z80miBC: case Z80mdBC: case Z80mBCi: case Z80mBCd:
					preinc(e,y); e.push(0x0a); postinc(e,y);
				break;
				case Z80mDE: case Z80miDE: case Z80mdDE: case Z80mDEi: case Z80mDEd:
					preinc(e,y); e.push(0x1a); postinc(e,y);
				break;
				case Z80mnn:
					e.push(0x3a); check16(y.val); e.push((byte)y.val);
					e.push((byte)(y.val>>8));
				break;
				case Z80_I:
					e[0]=0xed; e[1]=0x57;
		...
		case Z80_B: case Z80_C: case Z80_D: case Z80_E: case Z80_H: case Z80_L:
			switch (y.reg) {
				case Z80mHL: case Z80miHL: case Z80mdHL: case Z80mHLi: case Z80mHLd:
					preinc(e,y); e.push(0x46+x.reg*8); postinc(e,y);
				break;
				case Z80mIX: case Z80miIX: case Z80mdIX: case Z80mIXi: case Z80mIXd:
					preinc(e,y); e.push(0xdd); e.push(0x46+x.reg*8);
					e.push(checki(y.val)); postinc(e,y);
				break;
				case Z80mIY: case Z80miIY: case Z80mdIY: case Z80mIYi: case Z80mIYd:
					preinc(e,y); e.push(0xfd); e.push(0x46+x.reg*8);
					e.push(checki(y.val)); postinc(e,y);
				break;
				case Z80_A: case Z80_B: case Z80_C: case Z80_D:
				case Z80_E: case Z80_H: case Z80_L:
				...
				break;
				case Z80mnn: if (y.ind==2) { error(EM_Indirection_not_allowed); break; }
				case Z80_nn:
				  	e.push(0x06+x.reg*8); e.push(check8(y.val));
				break;
				case Z80_IXH:
					if (x.reg==Z80_H || x.reg==Z80_L) break;
					e.push(0xdd); e.push(0x44+x.reg*8);
				break;
				case Z80_IXL:
					if (x.reg==Z80_H || x.reg==Z80_L) break;
					e.push(0xdd); e.push(0x45+x.reg*8);
				break;
				case Z80_IYH:
					if (x.reg==Z80_H || x.reg==Z80_L) break;
					e.push(0xfd); e.push(0x44+x.reg*8);
				break;
				case Z80_IYL:
					if (x.reg==Z80_H || x.reg==Z80_L) break;
					e.push(0xfd); e.push(0x45+x.reg*8);
				break;
				default: break;
			}
		break;
		case Z80_SP:
			...
			case Z80_BC: case Z80_DE: case Z80_HL:
				switch (y.reg) {
					case Z80mnn:
						if (x.reg==Z80_HL) { e.push(0x2a); break; }
						e.push(0xed); e.push(0x3b+x.reg);
					break;
					case Z80_nn:
						e.push(x.reg-15);
					break;
			...
			case Z80_IX: case Z80_IY:
				switch (y.reg) {
					case Z80mnn:
						e.push(x.reg); e.push(0x2a);
					break;
					case Z80_nn:
						e.push(x.reg); e.push(0x21);
					break;
					default: break;
				}
				if (!e.size()) break;
			...
			case Z80_IXH: case Z80_IXL: case Z80_IYH: case Z80_IYL:
				switch (y.reg) {
					...
					case Z80_nn:
						switch (x.reg) {
							case Z80_IXH:
								e.push(0xdd); e.push(0x26); e.push(check8(y.val));
							break;
							case Z80_IXL:
								e.push(0xdd); e.push(0x2e); e.push(check8(y.val));
							break;
							case Z80_IYH:
								e.push(0xfd); e.push(0x26); e.push(check8(y.val));
							break;
							case Z80_IYL:
								e.push(0xfd); e.push(0x2e); e.push(check8(y.val));
							break;
							default: break;
						}
					break;
					case Z80_IXH:
						if (y.reg==Z80_IXH) {
							if (x.reg==Z80_IYH || x.reg==Z80_IYL) break;
							else y.reg=Z80_H;
						}
					case Z80_IXL:
						if (y.reg==Z80_IXL) {
					...
					case Z80_IYH:
						if (y.reg==Z80_IYH) {
							if (x.reg==Z80_IXH || x.reg==Z80_IXL) break;
							else y.reg=Z80_H;
						}
					case Z80_IYL:
						if (y.reg==Z80_IYL) {
	...
	if (!e.size()) error(EM_Illegal_operand,ERRREP);
	checkjunk(line);
}
	...
	switch (x.reg) {
		case Z80_A:
			switch (y.reg) {
				...
				case Z80mIX: case Z80miIX: case Z80mdIX: case Z80mIXi: case Z80mIXd:
					preinc(e,y); e.push(0xdd); e.push(0xb6);
					e.push(checki(y.val)); postinc(e,y);
				break;
				case Z80mIY: case Z80miIY: case Z80mdIY: case Z80mIYi: case Z80mIYd:
					preinc(e,y); e.push(0xfd); e.push(0xb6);
					e.push(checki(y.val)); postinc(e,y);
				break;
				default: error(EM_Illegal_operand,ERRREP); break;
			}
...
void pmOUT( string line,Data &e ) {
	...
	switch( x.reg ) {
		...
		break;
		case Z80mnn:
			if (y.reg!=Z80_A) { error(EM_Illegal_operand,ERRREP); break; }
	...
	switch (x.reg) {
		...
		case Z80_nn:
			...
			switch (y.reg) {
				...
				case Z80mIX: case Z80miIX: case Z80mdIX: case Z80mIXi: case Z80mIXd:
					preinc(e,y); e.push(0xdd); e.push(0xcb); e.push(checki(y.val));
					e.push(undoc(line,8*bit+0x80)); postinc(e,y);
				break;
				case Z80mIY: case Z80miIY: case Z80mdIY: case Z80mIYi: case Z80mIYd:
					preinc(e,y); e.push(0xfd); e.push(0xcb); e.push(checki(y.val));
					e.push(undoc(line,8*bit+0x80)); postinc(e,y);
				break;
				default: error(EM_Illegal_operand,ERRREP); break;
			}
...
void pmRL( string line, Data &e ) {
	...
	switch (x.reg) {
		...
		case Z80mIX: case Z80miIX: case Z80mdIX: case Z80mIXi: case Z80mIXd:
			preinc(e,x); e.push(0xdd); e.push(0xcb);
			e.push(checki(x.val)); e.push(undoc(line,0x10)); postinc(e,x);
		break;
		case Z80mIY: case Z80miIY: case Z80mdIY: case Z80mIYi: case Z80mIYd:
			preinc(e,x); e.push(0xfd); e.push(0xcb);
			e.push(checki(x.val)); e.push(undoc(line,0x10)); postinc(e,x);
		break;
		default: error(EM_Illegal_operand,ERRREP); break;
	}
...
void pmRLC( string line, Data &e ) {
	...
	switch (x.reg) {
		...
		case Z80mIX: case Z80miIX: case Z80mdIX: case Z80mIXi: case Z80mIXd:
			preinc(e,x); e.push(0xdd); e.push(0xcb);
			e.push(checki(x.val)); e.push(undoc(line,0x00)); postinc(e,x);
		break;
		case Z80mIY: case Z80miIY: case Z80mdIY: case Z80mIYi: case Z80mIYd:
			preinc(e,x); e.push(0xfd); e.push(0xcb);
			e.push(checki(x.val)); e.push(undoc(line,0x00)); postinc(e,x);
...
void pmRR( string line, Data &e ) {
	...
	switch (x.reg) {
		case Z80_B: case Z80_C: case Z80_D: case Z80_E:
		case Z80_H: case Z80_L: case Z80_A:
			e[0]=0xcb; e[1]=0x18+x.reg;
		break;
		case Z80mHL: case Z80miHL: case Z80mdHL: case Z80mHLi: case Z80mHLd:
			preinc(e,x); e.push(0xcb); e.push(0x1e); postinc(e,x);
		break;
		case Z80mIX: case Z80miIX: case Z80mdIX: case Z80mIXi: case Z80mIXd:
			preinc(e,x); e.push(0xdd); e.push(0xcb); e.push(checki(x.val));
			e.push(undoc(line,0x18)); postinc(e,x);
		break;
		case Z80mIY: case Z80miIY: case Z80mdIY: case Z80mIYi: case Z80mIYd:
			preinc(e,x); e.push(0xfd); e.push(0xcb); e.push(checki(x.val));
			e.push(undoc(line,0x18)); postinc(e,x);
		break;
		default: error(EM_Illegal_operand,ERRREP); break;
	}
...
void pmRRC( string line, Data &e ) {
	...
	switch (x.reg) {
		case Z80_B: case Z80_C: case Z80_D: case Z80_E:
		case Z80_H: case Z80_L: case Z80_A:
			e[0]=0xcb; e[1]=0x08+x.reg;
		break;
		case Z80mHL: case Z80miHL: case Z80mdHL: case Z80mHLi: case Z80mHLd:
			preinc(e,x); e.push(0xcb); e.push(0x0e); postinc(e,x);
		break;
		case Z80mIX: case Z80miIX: case Z80mdIX: case Z80mIXi: case Z80mIXd:
			preinc(e,x); e.push(0xdd); e.push(0xcb);
			e.push(checki(x.val)); e.push(undoc(line,0x08)); postinc(e,x);
		break;
		case Z80mIY: case Z80miIY: case Z80mdIY: case Z80mIYi: case Z80mIYd:
			preinc(e,x); e.push(0xfd); e.push(0xcb);
			e.push(checki(x.val)); e.push(undoc(line,0x08)); postinc(e,x);
...
void pmRST( string line, Data &e ) {
	...
	getOperand(line,x);
	if (x.reg==Z80mnn) {
		if (x.ind>1) { error(EM_Indirection_not_allowed); return; }
		else x.reg=Z80_nn;
	}
	if (x.reg!=Z80_nn) { error (EM_Illegal_operand); return; }
	switch (x.val) {
	...
	switch (x.reg) {
		case Z80_A:
			switch (y.reg) {
				...
				case Z80mHL:  case Z80miHL: case Z80mdHL:
				case Z80mHLi: case Z80mHLd:
					preinc(e,y); e.push(0x9e); postinc(e,y);
				break;
				case Z80mIX:  case Z80miIX: case Z80mdIX:
				case Z80mIXi: case Z80mIXd:
					preinc(e,y); e.push(0xdd); e.push(0x9e);
					e.push(checki(y.val)); postinc(e,y);
				break;
				case Z80mIY:  case Z80miIY: case Z80mdIY:
				case Z80mIYi: case Z80mIYd:
					preinc(e,y); e.push(0xfd); e.push(0x9e);
					e.push(checki(y.val)); postinc(e,y);
				break;
				default: error(EM_Illegal_operand,ERRREP); break;
			}
	...
	switch (x.reg) {
		...
		case Z80_nn:
			...
			switch (y.reg) {
				case Z80_B: case Z80_C: case Z80_D: case Z80_E:
				case Z80_H: case Z80_L: case Z80_A:
					e[0]=0xcb; e[1]=8*bit+0xc0+y.reg;
				break;
				case Z80mHL: case Z80miHL: case Z80mdHL:
				case Z80mHLi: case Z80mHLd:
					preinc(e,y); e.push(0xcb);
					e.push(8*bit+0xc6); postinc(e,y);
				break;
				case Z80mIX:  case Z80miIX: case Z80mdIX:
				case Z80mIXi: case Z80mIXd:
					preinc(e,y); e.push(0xdd); e.push(0xcb);
					e.push(checki(y.val));
					e.push(undoc(line,8*bit+0xc0));
					postinc(e,y);
				break;
				case Z80mIY:  case Z80miIY: case Z80mdIY:
				case Z80mIYi: case Z80mIYd:
...
void pmSLA( string line, Data &e ) {
	...
	switch (x.reg) {
		case Z80_B: case Z80_C: case Z80_D: case Z80_E:
		case Z80_H: case Z80_L: case Z80_A:
			e[0]=0xcb; e[1]=0x20+x.reg;
		break;
		case Z80mHL: case Z80miHL: case Z80mdHL: case Z80mHLi: case Z80mHLd:
			preinc(e,x); e.push(0xcb); e.push(0x26); postinc(e,x);
		break;
		case Z80mIX: case Z80miIX: case Z80mdIX:
		case Z80mIXi: case Z80mIXd:
			preinc(e,x); e.push(0xdd); e.push(0xcb);
			e.push(checki(x.val));
			e.push(undoc(line,0x20));
			postinc(e,x);
		break;
		case Z80mIY: case Z80miIY: case Z80mdIY:
		case Z80mIYi: case Z80mIYd:
...
void pmSLL( string line, Data &e ) {
	...
	switch (x.reg) {
		case Z80_B: case Z80_C: case Z80_D: case Z80_E:
		case Z80_H: case Z80_L: case Z80_A:
			e[0]=0xcb; e[1]=0x30+x.reg;
			break;
		case Z80mHL: case Z80miHL: case Z80mdHL: case Z80mHLi: case Z80mHLd:
			preinc(e,x); e.push(0xcb); e.push(0x36); postinc(e,x);
		break;
		case Z80mIX: case Z80miIX: case Z80mdIX:
		case Z80mIXi: case Z80mIXd:
			preinc(e,x); e.push(0xdd); e.push(0xcb);
			e.push(checki(x.val)); e.push(undoc(line,0x30)); postinc(e,x);
		break;
		case Z80mIY: case Z80miIY: case Z80mdIY:
		case Z80mIYi: case Z80mIYd:
			preinc(e,x); e.push(0xfd); e.push(0xcb);
	...
	switch (x.reg) {
		case Z80_A:
			switch (y.reg) {
				...
				case Z80mHL: case Z80miHL: case Z80mdHL: case Z80mHLi: case Z80mHLd:
					preinc(e,y); e.push(0x96); postinc(e,y);
				break;
				case Z80mIX: case Z80miIX: case Z80mdIX: case Z80mIXi: case Z80mIXd:
					preinc(e,y); e.push(0xdd); e.push(0x96); e.push(checki(y.val)); postinc(e,y);
				break;
				case Z80mIY: case Z80miIY: case Z80mdIY: case Z80mIYi: case Z80mIYd:
					preinc(e,y); e.push(0xfd); e.push(0x96); e.push(checki(y.val)); postinc(e,y);
				break;
				default: error(EM_Illegal_operand,ERRREP); break;
			}
		break;
	...
	switch( x.reg ) {
		case Z80_A:
			switch (y.reg) {
				...
				case Z80mHL: case Z80miHL: case Z80mdHL: case Z80mHLi: case Z80mHLd:
					preinc(e,y); e.push(0xae); postinc(e,y);
				break;
				case Z80mIX: case Z80miIX: case Z80mdIX: case Z80mIXi: case Z80mIXd:
					preinc(e,y); e.push(0xdd); e.push(0xae); e.push(checki(y.val)); postinc(e,y);
				break;
				case Z80mIY: case Z80miIY: case Z80mdIY: case Z80mIYi: case Z80mIYd:
					preinc(e,y); e.push(0xfd); e.push(0xae); e.push(checki(y.val)); postinc(e,y);
				break;
				default: error(EM_Illegal_operand,ERRREP); break;
			}
		break;
...
void pimsx( string &line, SourceList *source ) {
	...
	if (!funp) { error1(EM_Unknown_instruction,d); return; }
