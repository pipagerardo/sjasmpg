...
// datastructures.h
#ifndef _SJASM_DATASTRUCTURES_
#define _SJASM_DATASTRUCTURES_
...
class Data {
public:
	Data() : _data(NULL), _size(0), _maxsize(4)  { _resize(_maxsize); }
	Data(int n_maxsize) : _data(NULL), _size(0), _maxsize(n_maxsize) { _resize(_maxsize); }
	Data(byte *n_data,int n_size) : _data(NULL), _size(n_size), _maxsize(n_size) { _resize(n_size); memcpy(_data,n_data,n_size); }
	~Data() { delete [] _data; }
