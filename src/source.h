...
#ifndef _SJASM_SOURCE_
#define _SJASM_SOURCE_
...
struct ListOpt {
	void init() {
		_list=true; _char=' '; _include=-1; _inc=true; _filename.clear(); _path.clear(); _macrocurlin=-1;
	}
...
public:
	SourceListLineInc( string n_line ) :
		_line(n_line) {
			++listcurlin; ++curlin; _listtype=LISTLINE;
		}
...
public:
	SourceListLineMacroInc(string n_line) :
		_line(n_line) {
			++listcurlin; ++listopt._macrocurlin; _listtype=LISTLINE;
		}
...
public:
	FieldLabel( int n_location, string n_expr ) :
		_expr(n_expr),
		_location(n_location) {
			_listtype=LISTINFO;
		}
	void Process();
...
public:
	EquLabel( int n_location, string n_expr ) :
		_expr(n_expr),
		_location(n_location) {
			_listtype=LISTINFO;
		}
...
public:
	SetLabel( int n_location, string n_expr ) :
		_expr(n_expr), _location(n_location) {
			_listtype=LISTINFO;
		}
	void Process();
	string listline() { return '('+tohex(_p,2)+':'+tohex(_a)+')'; }
...
public:
	Mnemonic() {}
	Mnemonic( string n_line, mFun n_funp ) :
		_line(n_line), _funp(n_funp) { _listtype=LISTDATA; }
...
public:
	dRout( int n_rnr, Rout *n_rout, string n_line ) :
	//	_page(page),
		_rnr(n_rnr),
		_rout(n_rout),
		_line(n_line) { _listtype=LISTINFO; }
	void Process();
private:
//	int _page, _rnr;    // _page sin usar
	int _rnr;
	Rout *_rout;
	string _line;
	int _p, _a;
...
public:
	dPage( int n_rnr, Rout *n_r, string n_line ) :
		_line(n_line), _drout(n_rnr, n_r, "") { _listtype=LISTINFO; }
	void Process();
	string listline() { return '('+tohex(_listinfo,2)+')'; }
private:
	string _line;
//	bool _infook;       // Sin uso
	int _listinfo;
	dRout _drout;
};
...
public:
	StructExp( string n_line, int n_structure, string n_label ) :
		_line(n_line), _label(n_label), _structure(n_structure) { _listtype=LISTDATA; }
	Data &listdata() { return _e; }
	void Process();
private:
	string _line,_label;
