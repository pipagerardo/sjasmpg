...
int Rout::pool (int d ) {
	for( int i=0;i!=(int)_pool.size();++i ) {
		if( _pool[i]==d ) return _pooladres+4*i;
	}
	_pool.push_back(d);
...
void Part::dump( int &adres, StringList &sl ) {
	if( adres!=_adres ) {
		string s= "   " + tohex(adres,8) + "  " + tostr(_adres-adres,6) + TXT_empty;
		sl.push_back(s);
	}
	...
	switch(_alignmode) {
		case NORMAL:
			if( _align == 1 ) s += "    ";
			else s += tostr( _align, 4 );
		break;
		case BYTEADR:  s+="   #"; break;
		case FIXEDADR: s+="   @"; break;
		case RANGEADR: s+="  .."; break;
		default: error( EM_Unknown_alignmode, ERRFATAL );
	}
	switch(_multipage) {
		case 0: s+="   "; break;
		case 1: s+=" < "; break;
		case 2: s+=" > "; break;
		default: error( EM_Unknown_multipagemode, ERRFATAL );
	}
    ...
    if( !line.empty() ) {
        if (sbcneed(line,'!')) {
			...
			if( !ParseExpression( line, val ) ) {
				error(EM_Overlay_code_part_needs);
			}
			_minadres=_maxadres=val;
			_adrmode=ATADRES;
			goto rout_set_break;
		}
		if (sbcneed(line,'?')) _forgetmode=DOFORGET;
		else _forgetmode=NEVERFORGET;
		comma(line);
		if (sbcneed(line,'@')) {
			if (sbcneed(line,'*')) {
				_adrmode=ANYADRES; _minadres=0; _maxadres=BIGVALUE;
				...
				synerr=0;
				if (ParseExpression(line,val)) _minadres=val;
				else _minadres=0;
				if (need(line,(char*)"..")) {
					if (ParseExpression(line,val)) _maxadres=val;
					else _maxadres=BIGVALUE;
					_adrmode=ARANGE;
				} else {
...
rout_set_break:
	if ((unsigned)_minadres>(unsigned)_maxadres) {
		_minadres=_maxadres; error(EM_Invalid_addressrange);
	}
	if (pass>1) {
		...
		switch(_adrmode) {
			...
			break;
			default:
				error(EM_Illegal_addressmode,ERRINTERNAL);
			break;
		}
	}
	checkjunk(line);
...
int Rout::makepart( bool &overlay, int &cpagm, int &cadrm,int &npage,
	int &nadres,int &nlen, int &nalign, ALIGNMODE &nalignmode, int &nmultipage )
{
	overlay=_overlay;
	cadrm=(int)_adrmode;
//	_forget=false;
	...
	switch (_adrmode) {
		...
		case ARANGE:
			nadres = _maxadres - _minadres;
			if( _align == 1 ) nalignmode = RANGEADR;
		break;
		case ANYADRES:
			nadres=BIGVALUE;
			if( _multipage ) cadrm = (int)ANYADRESMULTI;
		break;
		default: break;
	}
...
int align(int adres, int align, int len, ALIGNMODE alignmode) {
	...
	if (adres%align) return adres+(align-adres%align);
//	else return adres;
	return adres;
}

void Page::setsize( int n_size ) {
	if (_size!=-1) error( EM_Size_already_set );
	_size=n_size;
}

void Page::setorg( int org ) {
	if( _relorg ) {
		if( _org!=org ) labsnok=1;
		_org=org;
	}
}
...
int Page::addpart( Part &n_part ) {
	...
	minadres = maxadres = 0;
	list<Part> *lp;
	if (n_part._rout->overlay()) lp=&_opart;
	else lp=&_part;
	...
	if (lp->empty()) {
		...
		lp->push_back(n_part);
		ap+=n_part._len;
		if( ap>_highestad ) _highestad=ap;
		return 1;
	}
	ap=_org;
	...
	while (ip!=lp->end()) {
		if( align( ap, n_part._align, n_part._len, n_part._alignmode ) +
			n_part._len <= ip->_adres
		) break;
...
bool Page::addfirstpart( Part &n_part, int offset, int size ) {
	_part.push_front(
		Part( n_part._rnr, _pagenr, _org, size, n_part._align,
			n_part._alignmode, n_part._rout, 2
		)
	);
...
void Page::checkparts() {
	for( list<Part>::iterator ip=_part.begin(); ip!=_part.end(); ++ip ) {
		...
		if( ipn!=_part.end() )
			if( ip->_adres+ip->_len>ipn->_adres )
				error( EM_Overlapping_routines, ERRINTERNAL);
	}
...
void Page::dump( StringList &sl ) {
	int adres=_org;
	string s="  Org: " + tohex(_org,8)+ TXT_Size;
	if (_size==-1) s+="*"; else s+=tohex(_size,8);
	s+=TXT_Used + tohex(_usedsize,8);
	sl.push_back(s);
	sl.push_back("");
	if (_part.size()) {
		sl.push_back( TXT_Address_Length_Label );
		for( list<Part>::iterator ip=_part.begin(); ip!=_part.end(); ++ip )
			ip->dump( adres, sl );
		if( _size!=-1 && adres!=_org+_size ) {
			string s= "   " + tohex(adres,8) + "  " + tostr(_org+_size-adres,6) + TXT_empty;
			sl.push_back(s);
		}
	} else {
		if (_size==-1) sl.push_back( TXT_No_output );
		else sl.push_back( "   " + tohex(_org,8) + "  " + tostr(_size,6) + TXT_empty );
	}
	if (_opart.size()) {
		sl.push_back("");
		sl.push_back( TXT_Overlay_parts );
		sl.push_back( TXT_Address_Length_Label );
...
//			break;	// ???
			default: error( "Oops!", ERRFATAL ); break;
		}
...
void Output::defpage( int n_page, int n_org, int n_size ) {
	...
	if (_page[n_page]) error(EM_Page_already_exists);
	_page[n_page]=new Page(n_page,n_org, n_size);
}

void Output::setpagesize( int n_size ) {
	if( _page.empty() ) defpage( 0, output[onr]->getorg(), n_size );
	else _page[page]->setsize( n_size );
}

int Output::getpagesize( int page ) {
	if ((unsigned)page>=_page.size()) {
		error(EM_Page_does_not_exist,tostr(page));
		return 0;
	}
...
void Output::dump(StringList &sl) {
	sl.push_back("");
	sl.push_back( TXT_Output +_filename);
	sl.push_back("-------------------------------------------------");
	for( int i=0; i!=(int)_page.size(); ++i ) {
		if (_page[i]) {
			sl.push_back(""); sl.push_back( TXT_Page + tohex(i,2) );
			_page[i]->dump(sl);
		}
	}
}
...
void Output::sort() {
	...
	while (ip!=p.end()) {
		...
		if (ip->_alignmode==BYTEADR && ip->_len>256 && pass>2) {
			ip->_rout->geterrlin();
			error( EM_Maximum_part_size_256,
				labtab.getname( ip->_rout->getlabelindex() )
			);
			labsnok=1;
		}
		...
		if (!ok && pass>2) {
			...
			string es=TXT_Part,name=labtab.getname(ip->_rout->getlabelindex());
			if (!name.empty()) es+=" '"+name+"'";
			es += TXT_does_not_fit_in_page;
			...
			error(es);
//			error(EM_Part_does_not_fit,labtab.getname(ip->_rout->getlabelindex()));
			labsnok=1;
...
void Output::save() {
	...
	if( file.ok() ) {
		for(int i=0; i!=(int)_page.size(); ++i) {
			...
			else if (options.allpages) error(EM_Page_does_not_exist,tostr(i));
		}
	}
	}
	...
	if( file.ok() ) {
		for( int i=0; i!=(int)_page.size(); ++i ) if (_page[i]) _page[i]->saveoverlay(file);
	}
	}
}
