...
// output.h
#ifndef _SJASM_OUTPUT_
#define _SJASM_OUTPUT_
...
public:
	Rout() :
		_adres(adres), _page(page), _lastused(0), _pooladres(adres), _multipage(false),
		_forget(false), _overlay(false), _byteadr(false),
		_minadres(adres),  _maxadres(0),  _align(1),
		_adrmode(ANYADRES), _forgetmode(NEVERFORGET),
		_labelindex(0), _curlin(curlin), _listcurlin(listcurlin),
		_sourcefile(listopt._filename) {
			routlabel=this;
			_pages.clear();
...
class Part {
public:
	Part(int n_rnr, int n_page, int n_adres, int n_len, int n_align, ALIGNMODE n_alignmode, Rout *n_rout, int n_multipage) :
		_alignmode(n_alignmode),
		_page(n_page), _adres(n_adres), _len(n_len), _align(n_align), _rnr(n_rnr),
		_multipage(n_multipage), _offset(0),
		_rout(n_rout) {}
...
public:
	Page(int n_pagenr, int n_org, int n_size) :
		_pagenr(n_pagenr), _org(n_org), _size(n_size), _highestad(0), _usedsize(0) {
...
private:
//	struct _Pool {
//		Pool (int n_data,int n_adres) : _data(n_data), _adres(n_adres) {}
//		int _data, _adres;
//	};
...
public:
	Output(string n_filename,FILEMODE n_mode=OVERWRITE) :
		_filename(n_filename), _mode(n_mode), _rn(0), _org(-1), _outp(NULL) {
		_page.clear(); _rout.clear();
	}
	Rout *getroutp() {
		if( _rout.empty() ) {
	...
	void setrout(int n_rn) {
		_outp=_rout[_rn=n_rn]; // <- Posible excepción.
	}
	void defpage(int,int,int);
	void setpagesize(int);
	int getpagesize(int);
	void setorg( int n_org ) {
		if( _org==-1 ) _org=n_org;
	}
	int getorg() {
		if( _org==-1 ) return 0;
		return _org;
	...
	int getpageorg( int p ) {
		if( _page.empty() || (unsigned)p>_page.size() ) return 0;
		try {
			if( _page[ p ] == NULL ) throw 'p';
		} catch(...) {
			// error( "Page not defined: ", p );
			return 0;
		}
		return _page[ p ]->getorg();
	}
	int pageok( int np ) {
		if( _page.empty() || (unsigned)np>=_page.size() ) return 0;
		try {
			if( _page[ np ] == NULL ) throw "NP";
		} catch(...) {
			return 0;
		}
		return _page[np]!=NULL;
	}
	void emit(Data &e) {
		if( !e.size() ) return;
		if (!_outp) getroutp();
