...
string DefineArgTable::getrepl(string naam, int skip) {
	...
	int p=_nextlocation;
	while( --p ) {
		if (naam==_deftab[p]._name && !skip--) return _deftab[p]._repl;
	}
	return "";
}
...
string MacNumTable::getrepl( int num, int skip ) {
	int p=_nextlocation;
	while (--p) {
		if (num==_numtab[p]._num && !skip--) return _numtab[p]._repl;
	}
	return "";
}
...
void DefineTable::add( string n_name, string n_repl ) {
	...
	if (p==-1) { error(EM_Overloading_not_allowed); return; }
	if (p) {
		...
		while( (htr=_hashtable[tr]) ) {
			if (_deftab[htr]._name==n_name && _deftab[htr]._args.size()==0)
				error( "DefineTable::add", ERRINTERNAL );
...
void DefineTable::add( string n_name, string n_repl, bool n_icase, StringList n_args ) {
	...
	if (p==-1) { error(EM_Overloading_not_allowed); return; }
	if (p) {
		if (n_args.size()) { error(EM_Redefinition_multiparamet); return; }
		_deftab[p]._repl=n_repl;
	} else {
		...
		tr=_hash(n_name);
		while( (htr=_hashtable[tr]) ) {
			if( _deftab[htr]._name==n_name &&
				_deftab[htr]._args.size()==n_args.size()
			) error( "DefineTable::add", ERRINTERNAL );
			else if (++tr>=_size) tr=0;
		}
		_hashtable[tr]=_nextlocation;
...
int DefineTable::exists( string name, int narg ) {
	...
	otr=tr=_hash(name);
	while( (htr=_hashtable[tr]) ) {
		if( _deftab[htr]._name==name &&
			_deftab[htr]._icase && !_deftab[htr]._used )
		{
			if (narg) {
				if (!_deftab[htr]._args.size()) return -1;
				if (narg==(int)_deftab[htr]._args.size()) return htr;
...
void DefineTable::undef( string s ) {
	...
	else error(EM_Cant_undef_a_define_with,s);
}
...
void LabelTable::setvalue( int loc, int nval, int npage, bool nset ) {
	if (nset!=_labtab[loc]._set) error1(EM_Mixing_set_normal_labels);
...
void LabelTable::listdump( StringList &sl ) {
	string s;
	sl.push_back("");
	sl.push_back( TXT_LABELS );
	sl.push_back("-------------------------------------------------");
	for (int i=1; i<_nextlocation; ++i) {
		if (!isdigit(_labtab[i]._name[0])) {
			s.clear();
			s = tohex( _labtab[i]._page, 2 ) +
				':' + tohex( _labtab[i]._val, 8 ) +
				"   " + _labtab[i]._name;
...
void MacroTable::add( string n_name, bool n_icase, Macro n_mac ) {
	...
	int p=exists(n_name);
	if (p) {
		if (_mactab[p]._icase!=n_icase) {
			error( EM_Macro_already_exists ); return;
		}
	} else {
		if (n_icase) n_name=tolower(n_name);
		int tr,htr;
...
int MacroTable::emit( string &line, RawSource *rs, bool fun ) {
	...
	for(;;) {
		...
		if (narg==max) args.push_back(getargument(p,im->_greedy,fun));
		else args.push_back(getargument(p,false,fun));
		if (!comma(p)) break;
	}
	if (narg<im->_minparam) return 0;
	if (im->_used && !im->_recursive && !fun) return 0;
	if (fun && !sbcneed(p,')')) error(EM_Closing_P_expected);
	...
	for (i=0; i!=(int)im->_namParam.size(); ++i) {
		if (ia==args.end()) {
			...
			if (a=="\01") { a=" "; error(EM_Parameter_missing); }
			defargtab.add(im->_namParam[i],a);
		} else {
			if (*ia=="") {
				*ia=im->_namDefaults[i];
				if (*ia=="\01") { *ia=" "; error(EM_Parameter_missing); }
			}
	...
	for (i=1; i<=im->_minnum; ++i) {
		if (ia==args.end()) { error(EM_Parameter_missing); break; }
...
void MacroTable::_macroEntry::add( Macro n_macro ) {
	if (_macro.size()) {
		for (list<Macro>::iterator im=_macro.begin(); im!=_macro.end(); ++im) {
			if (
				(n_macro._maxparam>=im->_minparam && n_macro._maxparam<=im->_maxparam) ||
...
void Structure::emitdata( string line, Data &e, string label ) {
	...
	for (vector<_Member*>::iterator im=_memtab.begin(); im!=_memtab.end(); ++im) {
		...
		if (!label.empty() && !(*im)->_name.empty()) {
			string n;
			if ((*im)->_name[0]=='.') n=label+(*im)->_name;
			else n=label+'.'+(*im)->_name;
			int p=labtab.getindex(n);
			if (p) labtab.setvalue(p,adres+(*im)->_offset,page);
			else error( EM_Label_structure_not_found, ERRINTERNAL);
		}
		d.clear();
		switch ((*im)->_type) {
			case 0: break;
		...
		if (d.empty()) {
			...
			switch((*im)->_type) {
				case 0: break;
				case 1:
					if( !ParseExpression(d,val) ) error(EM_Syntax_error);
					checkjunk(d);
				...
				case 3:
					...
					checkjunk(d);
					val=check24(val);
					for (int i=0;i!=3;++i) {e.push((byte)val); val>>=8; }
				break;
				case 4:
					if (!ParseExpression(d,val)) error(EM_Syntax_error);
					checkjunk(d);
				...
				case 5:
					...
					if (f.size()>(*im)->_length) error(EM_Too_much_data);
					e.push(f);
				break;
				default:
					error(EM_No_default_allowed);
				break;
			}
		}
		offset+=(*im)->_length;
...
int StructTable::add( Structure ns ) {
	if (_nextlocation>=_size*2/3) _grow();
	if(exists(ns._name)) { error(EM_Structure_already_exists,ns._name); return 0; }
	int tr,htr;
	tr=_hash(ns._name);
	while( (htr=_hashtable[tr]) ) {
