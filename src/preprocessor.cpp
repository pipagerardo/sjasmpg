...
string getbuildindefine( string d, string & ) {
	...
	if (d=="__MODULE__") {
		if (modlabp.empty()) return " ";
		else return modlabp;
	}
	if (d=="__VERSION__") return VERSION_NUM;
...
Find frepdef(
	(char*)"qwertyuioplkjhgfdsazxcvbnmQWERTYUIOPLKJHGFDSAZXCVBNM_'\"@"
);
...
string ReplaceDefines( string s ) {
	...
	for(;;) {
		...
		if (s[0]=='_' || isalpha(s[0])) {
			...
			if (s[0]!='@') error1(EM_ReplaceDefines,ERRINTERNAL);
			int skip=-1;
			while (cneed(s,'@')) ++skip;
			if (s[0]=='\'' || s[0]=='"') {
				pos=getstringlength(s);
				...
				int max,num,rot;
				if (!getConstant(s,num)) error1(EM_ReplaceDefines,ERRINTERNAL);
				nr=macnumtab.getrepl(0,skip);
				if (nr.empty()) { error1( EM_Use_num_outside_macro ); continue; }
				if (num) {
					if (!getConstant(nr,max)) error1(EM_ReplaceDefines,ERRINTERNAL);
					if (num>max) nr="";
					else {
						nr=macnumtab.getrepl(ROTATE,skip);
						if (nr.empty()) error1(EM_ReplaceDefines,ERRINTERNAL);
						if (!getConstant(nr,rot)) error1(EM_ReplaceDefines,ERRINTERNAL);
				...
				if (nr.empty()) { nr="0"; error1(EM_Use_num_outside_loops); }
				res+=nr;
			} else if (cneed(s,'*')) {
...
int pregetdef( string &line, string &n, string &r, StringList &a ) {
	a.clear();
	if ((n=getid(line)).empty()) { error1(EM_Illegal_define_name); return 0; }
	if (line[0]=='(') {
		line.erase(0,1);
		while(1) {
...
void elseif( bool c ) {
	if (ifstack.empty()) { error1(EM_ELSEIF_without_IF); return; }
	if (ifstack.top()._else) { error1(EM_ELSEIF_within_ELSE); return; }
...
void preELSE( string &, RawSource * ) {
	if (ifstack.empty()) { error1(EM_ELSE_without_IF); return; }
	if (ifstack.top()._else) { error1(EM_ELSE_within_ELSE); return; }
...
void ifIF( string &line, void(*push)(bool) ) {
	int val;
	if (!ParseExpression(line,val)) { error1(EM_Expression_expected); return; }
	if (labelnotfound) error1(EM_Forward_reference);
	(*push)(val!=0);
}
...
void preUNDEF( string &line, RawSource * ) {
	...
	else error(EM_Operand_expected);
}
...
void preASSIGN( string &line, RawSource * ) {
	...
	line.clear();
	int val;
	if (!ParseExpression(p,val)) error1(EM_Expression_expected);
	if (labelnotfound) error1(EM_Forward_reference);
	checkjunk(p);
...
void preSUBSTR( string &line, RawSource * ) {
	...
	if(comma(line)) {
		if(!ParseExpression(line,pos)) {
			error(EM_Syntax_error); pos=1;
		}
		if(comma(line)) {
			if(!ParseExpression(line,len)) {
				error(EM_Syntax_error); len=BIGVALUE;
			}
		}
	}
	checkjunk(line);
...
int pregetmacro( string &line, bool icase, RawSource *rs, bool list, bool recursive ) {
	...
	if (name.empty()) { error1(EM_Illegal_macroname); return 0; }
	while('>') {
		skipblanks(line);
		if (!line[0]) break;
		if (isalpha(line[0])) {
		...
		if (isdigit(line[0])) {
			if (!getConstant(line,mac._minnum)) { error1(EM_Constant_expected); return 0; }
			if (need(line,(char*)"..")) {
				skipblanks(line);
				if (sbcneed(line,'*'))
					mac._maxnum=BIGVALUE;
				else {
					if (!getConstant(line,mac._maxnum)) { error1(EM_Constant_expected); return 0; }
				}
			} else mac._maxnum=mac._minnum;
			if (mac._minnum<0 || mac._maxnum<mac._minnum) { error1(EM_Illegal_parameter); return 0; }
			if (sbcneed(line,'+')) {
				if (mac._maxnum<BIGVALUE) mac._greedy=true;
				else error1(EM_No_greedy_stars_allowed);
			}
			break;
		}
		if (sbcneed(line,'*')) {
			mac._minnum=0;
			mac._maxnum=BIGVALUE;
			if (sbcneed(line,'+')) error1(EM_No_greedy_stars_allowed);
			break;
		}
		error1(EM_Syntax_error);
		break;
	}
...
void premacrodot( string &line, RawSource *rs, bool icase ) {
	...
	while (!line.empty() && line[0]=='.') {
		...
		else error(EM_Unknown_instruction,"macro."+p);
	}
	if( !pregetmacro(line,icase,rs,list,recursive) )
		error1( EM_Illegal_macro_definition );
}
...
void preENDMACRO( string &, RawSource * ) {
	error1(EM_ENDMACRO_without_MACRO);
}
...
void preROTATE( string &line, RawSource * ) {
	...
	int val,rot,max;
	if (!ParseExpression(line,val)) { error1(EM_Expression_expected); return; }
	if (labelnotfound) { error1(EM_Forward_reference); return; }
	nr=macnumtab.getrepl(ROTATE);
	if (nr.empty()) { error1(EM_Use_rotate_outside_macro); return; }
...
void prewhile( string &line, RawSource *rs, bool list ) {
	...
	if (!ParseExpression(p,val)) error1(EM_Expression_expected,line);
	if (labelnotfound) error1(EM_Forward_reference,line);
	checkjunk(p);
	...
	if (errors<=oerrors) {
		...
		for (;;) {
			...
			p=line;
			if (!ParseExpression(p,val)) { error1(EM_Expression_expected,line); return; }
			if (labelnotfound) { error1(EM_Forward_reference,line); return; }
			if (!val) break;
...
void preWHILEDOT( string &line, RawSource *rs ) {
	...
	while (!line.empty() && line[0]=='.') {
		...
		else error(EM_Unknown_instruction,"while."+p);
	}
	prewhile(line,rs,list);
}

void preENDWHILE( string &, RawSource * ) {
	error1(EM_ENDWHILE_without_WHILE);
}
...
void prerepeat( string &line, RawSource *rs, bool list ) {
	...
	if (val<0) error1( EM_Negative_repeat_value );
...
void preREPEATDOT( string &line, RawSource *rs ) {
	...
	while (!line.empty() && line[0]=='.') {
		...
		else error(EM_Unknown_instruction,"repeat."+p);
	}
	prerepeat(line,rs,list);
}

void preENDREPEAT( string &, RawSource * ) {
	error1(EM_ENDREPEAT_without_REPEAT);
}

void preBREAK( string &, RawSource * ) {
	if (macnumtab.getrepl(ITERATION).empty()) error1(EM_BREAK_used_outside_loop);
	else stop._stop=BREAK;
}

void preCONTINUE( string &, RawSource * ) {
	if (macnumtab.getrepl(ITERATION).empty()) error1(EM_CONTINUE_outside_loop);
	else stop._stop=CONTINUE;
}

void preEXITMACRO( string &line, RawSource * ) {
	if (maclabp.empty()) error1(EM_EXITMACRO_outside_macro);
...
void preXEXITMACRO( string &line, RawSource * ) {
	int val;
	if (maclabp.empty()) error1( EM_XEXITMACRO_outside_macro );
	else stop._stop=EXITMACRO;
...
void preSTRUCT( string &line, RawSource *rs ) {
	...
	string n=getid(line); if (n.empty()) error1(EM_Illegal_structure_name);
	if (comma(line)) {
		switch(comma(line)) {
			case 0:
				if (!ParseExpression(line,offset)) error1( EM_Offset_expected);
				if (labelnotfound) error1(EM_Forward_reference);
				if (!comma(line)) break;
			default:
				if (!ParseExpression(line,align)) error1( EM_Alignment_expected );
				if (labelnotfound) error1(EM_Forward_reference);
				if (align<1) { align=1; error1(EM_Negative_zero_alignment); }
		}
	}
	checkjunk(line);
	int scurlin=curlin, slistcurlin=listcurlin;
	StringList sl=rs->ReadUntil( structurenl, structureel, "structure" );
	if (structtab.exists(n)) { error(EM_Structure_already_exists,n); return; }
	...
	else error(EM_Label_already_exists,n);
	structtab.add(s);
}

void preENDSTRUCT( string &, RawSource * ) {
	error1( EM_ENDSTRUCT_without_STRUCT );
}
...
void initpreprocessor() {
	FunctionEntry<pFun> group1if[] = {
		{(char*)"ifdef",preIFDEF},
		{(char*)"ifndef",preIFNDEF},
		{(char*)"else",preELSE},
		{(char*)"endif",preENDIF},
		{(char*)"elseifdef",preELSEIFDEF},
		{(char*)"elseifndef",preELSEIFNDEF}
	};
	FunctionEntry<pFun> group2if[] = {
		{(char*)"if",preIF},
		{(char*)"ifidn",preIFIDN},
		{(char*)"ifidni",preIFIDNI},
		{(char*)"ifdif",preIFDIF},
		{(char*)"ifdifi",preIFDIFI},
		{(char*)"ifb",preIFB},
		{(char*)"ifid",preIFID},
		{(char*)"ifstr",preIFSTR},
		{(char*)"ifnum",preIFNUM},
		{(char*)"ifnb",preIFNB},
		{(char*)"ifnid",preIFNID},
		{(char*)"ifnstr",preIFNSTR},
		{(char*)"ifnnum",preIFNNUM},
		{(char*)"ifexists",preIFEXISTS},
		{(char*)"ifnexists",preIFNEXISTS},
		{(char*)"ifin",preIFIN},
		{(char*)"ifini",preIFINI},
		{(char*)"elseif",preELSEIF},
		{(char*)"elseifidn",preELSEIFIDN},
		{(char*)"elseifidni",preELSEIFIDNI},
		{(char*)"elseifdif",preELSEIFDIF},
		{(char*)"elseifdifi",preELSEIFDIFI},
		{(char*)"elseifb",preELSEIFB},
		{(char*)"elseifid",preELSEIFID},
		{(char*)"elseifstr",preELSEIFSTR},
		{(char*)"elseifnum",preELSEIFNUM},
		{(char*)"elseifnb",preELSEIFNB},
		{(char*)"elseifnid",preELSEIFNID},
		{(char*)"elseifnstr",preELSEIFNSTR},
		{(char*)"elseifnnum",preELSEIFNNUM},
		{(char*)"elseifexists",preELSEIFEXISTS},
		{(char*)"elseifnexists",preELSEIFNEXISTS},
		{(char*)"elseifin",preELSEIFIN},
		{(char*)"elseifini",preELSEIFINI}
	};
	FunctionEntry<pFun> group1[] = {
		{(char*)"define",preDEFINE},
		{(char*)"idefine",preIDEFINE},
		{(char*)"xdefine",preXDEFINE},
		{(char*)"xidefine",preXIDEFINE},
		{(char*)"undef",preUNDEF},
		{(char*)"assign",preASSIGN},
		{(char*)"tostr",preTOSTR},
		{(char*)"strlen",preSTRLEN},
		{(char*)"substr",preSUBSTR}
	};
	FunctionEntry<pFun> group2[] = {
		{(char*)"include",preINCLUDE},
		{(char*)"macro",preMACRODOT},
		{(char*)"imacro",preIMACRODOT},
		{(char*)"rotate",preROTATE},
		{(char*)"while",preWHILEDOT},
		{(char*)"repeat",preREPEATDOT},
		{(char*)"break",preBREAK},
		{(char*)"continue",preCONTINUE},
		{(char*)"exitmacro",preEXITMACRO},
		{(char*)"xexitmacro",preXEXITMACRO},
		{(char*)"struct",preSTRUCT},
		{(char*)"macro.",preMACRODOT},
		{(char*)"imacro.",preIMACRODOT},
		{(char*)"while.",preWHILEDOT},
		{(char*)"repeat.",preREPEATDOT},
		{(char*)"endmacro",preENDMACRO},
		{(char*)"endwhile",preENDWHILE},
		{(char*)"endrepeat",preENDREPEAT},
		{(char*)"endstruct",preENDSTRUCT},
		{(char*)"endm",preENDMACRO},
		{(char*)"ends",preENDSTRUCT}
	};
