...
Find findlabelchar(
	(char*)"qazxswedcvfrtgbnhyujmkiolpQWERTYUIOPLKJHGFDSAZXCVBNM0123456789_."
);

Find findconstantchar( (char*)"_0123456789abcdefhoqABCDEFHOQ" );
...
int getLabelValue( string &ns, int &nval, int &npag ) {
	...
	if (!mlp.empty()) {
		if (cneed(s,'@')) mlp.clear();
			...
			do {
				...
				else {	// bug! .@.label wordt .label (moet ..label zijn)
	...
	n=s.substr(0,pos);
	if (!isalpha(n[0]) && n[0]!='.' && n[0]!='_') {
		error(EM_Illegal_label_name,s); return 0;
	}
	if (pos!=(int)string::npos) s=s.substr(pos);
	else s.clear();
	ns=s;
	if (!mlp.empty() && loc) {
		while(1) {
		//	if (labtab.getvalue(mlp+'>'+n,nval,npag)) return 1;
			if (labtab.getvalue(mlp+'>'+n,nval,npag)) {labelnotfound=0; return 1; }
			loc=(int)mlp.find_last_of('.');
			if (loc!=(int)string::npos) mlp=mlp.substr(0,loc);
			else break;
		}
...
//	if (labtab.getvalue(nn,nval,npag)) return 1;
	if (labtab.getvalue(nn,nval,npag)) {labelnotfound=0; return 1; }
//	if (!loc && !glb && labtab.getvalue(n,nval,npag)) return 1;
	if (!loc && !glb && labtab.getvalue(n,nval,npag)) {labelnotfound=0; return 1; }
	nval=0; labelnotfound=1;
	if (pass==1) labsnok=1;
	else error(EM_Label_not_found,nn);
	return 1;
}
...
int getval( string &p, int &nval, unsigned int base, int len ) {
	...
	while (len--) {
		...
		if (v!=(unsigned int)-1) {
			if (v>base) { error(EM_Digit_not_in_base,p); return 0; }
			ov=nv; nv=nv*base+v;
			if (ov>nv) error(EM_Overflow);
		}
		++pos;
	}
	nval=nv;
...
int getRepeatCount( string &p, int &v ) {
	...
	if (i<1) { error(EM_without_repeatblock); return 0; }
	v=repeatstack[i-1];
	return 1;
}
...
int ParseExpPrim( string &p, int &nval ) {
	...
	if (cneed(p,'(')) {
		res=ParseExp(p,nval);
		if (!sbcneed(p,')')) { error( EM_Closing_P_expected ); return 0; }
	} else if(
		isdigit(p[0]) || (p[0]=='#' && isalnum(p[1])) ||
		(p[0]=='$' && isalnum(p[1])) || ( p[0]=='%' && isdigit(p[1]) )
	) {
		return getConstant(p,nval);
	} else if (isalpha(p[0]) || p[0]=='_' || p[0]=='.' || p[0]=='@') {
		return getLabelValue(p,nval,dummy);
	} else if (cneed(p,'$')) {
		nval=adres; again=1; return 1;
	} else if (cneed(p,'#')) {
		nval=page; again=1; return 1;
	} else if (cneed(p,'%')) {
		return getRepeatCount(p,nval);
	} else if (cneed(p,':')) {
		if (cneed(p,':')) {
			res=ParseExp(p,nval);
			nval=output[onr]->getpagesize(nval);
			again=1;
			return 1;
		} else return getLabelValue(p,dummy,nval);
	} else if (!(res=getCharConst(p,nval))) {
		if( synerr ) error(EM_Syntax_error,ERRREP);
		return 0;
	}
	return res;
}
...
int ParseExpUnair(string &p, int &nval) {
	...
	int oper;
	if( (oper=need( p, (char*)"! ~ + - ") ) ||
		(oper=needa( p, (char*)"not",'!',"low",'l',"high",'h' ) )
	) {
		switch (oper) {
			...
			default: error(EM_Hoppa_Noppa,ERRINTERNAL);
		}
		return 1;
...
int ParseExpPow( string &p, int &nval ) {
	...
	while( need( p, (char*)"**" ) ) {
		...
		if( right<0 ) nval=0;
			...
			for( nval=right&1?left:1,right>>=1;right;right>>=1 ) {
				left*=left;
				if (right&1) nval*=left;
			}
		}
		left=nval;
	}
...
int ParseExpMul( string &p, int &nval ) {
	...
	while( (oper=need(p,(char*)"* / % ")) || (oper=needa(p,"mod",'%')) ) {
		if (!ParseExpPow(p,right)) return 0;
		switch (oper) {
			case '*': left*=right; break;
			case '/':
				if (right) left/=right;
				else { cout << EM_Division_by_zero; left=0; }
			break;
			case '%':
				if (right) left%=right;
				else { cout << EM_Division_by_zero; left=0; }
...
int ParseExpShift( string &p, int &nval ) {
	...
	while ((oper=need(p,(char*)"<<>>")) || (oper=needa(p,"shl",'<'+'<',"shr",'>'))) {
		...
		switch (oper) {
			...
			case '>'+'@':
				if (right>=32) left=0;
				else { unsigned long l=left; l>>=right; left=l; }
			break;
			default: error(EM_Hoppa_Noppa,ERRINTERNAL);
		}
	}
...
int ParseExpBitAnd( string &p, int &nval ) {
	...
	while (need(p,(char*)"&_") || needa(p,(char*)"and",'&')) {
...
int ParseExpBitOr( string &p, int &nval ) {
	...
	while (need(p,(char*)"|_") || needa(p,"or",'|')) {
