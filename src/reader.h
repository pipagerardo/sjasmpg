...
// reader.h
#ifndef _SJASM_READER_
#define _SJASM_READER_
...
template <class T> string tostr( T n, int w=0 ) { 
	ostringstream s;
	s.width(w);
	s << n;
	return s.str();
...
public:
	Find(char*s) {
		for( int i=0; i!=256; ++i ) _c[i]=false;
		while( *s ) _c[(size_t)*s++]=true;
	}
	int find( string&s ) {
		for( int i=0; i!=(int)s.size();++i )
			if( _c[(size_t)s[i]] ) return i;
		return (int)string::npos;
	}
	int findnot( string&s ) { 
		for( int i=0;i!=(int)s.size();++i )
			if( !_c[(size_t)s[i]] ) return i;
		return (int)string::npos;
	}
