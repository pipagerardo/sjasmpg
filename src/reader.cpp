...
Find fid(
	(char*)"qwertyuioplkjhgfdsazxcvbnmQWERTYUIOPLKJHGFDSAZXCVBNM1234567890_"
);
...
void checkjunk( string &s ) {
	...
	if (s.empty()) return;
#ifdef _DEBUG
		error(EM_Unexpected,s);
#else
		error(EM_Unexpected,s,ERRREP);
#endif
	s.clear();
}
...
void checkparen( string &s ) {
	...
	while (1) {
		switch (s[p]) {
			...
			case '(': ++t; break;
			case ')':
				if (t==1) { ++p; s=s.substr(p); skipblanks(s); return; }
				if (!--t) return;
			break;
			case '\'':
				for(;;) { ++p; if (!s[p]) return; if (s[p]=='\'') break; }
			break;
			case '"':
				for(;;) {
					++p;
					if (!s[p]) return;
					if (s[p]=='\\' && s[p+1]=='"') { ++p; continue; }
					if (s[p]=='"') break;
				}
			break;
			default: break;
...
int checki( int val ) {
	if (val<-128 || val>127) errorvalue(EM_Offset_out_of_range);
	return val & 255;
}
...
void makestring( string &s ) {
	string res;
	for(istring i=s.begin();i!=s.end();++i) {
		if (*i=='"' || *i=='\\') res+='\\';
		res+=*i;
	}
	s=res;
...
void getstring( string &s, string &e ) {
	e.clear();
	skipblanks(s);
	if (s.empty()) { error( EM_String_expected ); return; }
	if (s[0]=='\'') {
		for(;;) {
			s.erase(0,1);
			if (s.empty()) break;
		...
		do {
			int val;
			if (s.empty() || s[0]=='"') { error(EM_Syntax_error,ERRREP); break; }
			getcharconstchar(s,val); e.push_back((byte)val);
		} while( s[0]!='"' );
		s.erase(0,1);
	} else error( EM_String_expected );
}
...
void getcharconstchar( string &op, int &val ) {
	...
	switch (val) {
		...
		default: val='\\'; error(EM_Unknown_escape); return;
	}
}
...
StringList getarguments( string &p ) {
	...
	for(;;) {
		switch( p[0] ) {
			case 0:
				if (heind) error( EM_Closing_P_expected );
...
void getpage( string &s, IntList &p ) {
	...
	while(1) {
		...
		if ((unsigned)min>255) { min=255; error(EM_Illegal_pagenumber); }
		if (need(s,(char*)"..")) {
			if (ParseExpression(s,v)) max=v;
			else max=255;
			if ((unsigned)max>255) { max=255; error(EM_Invalid_pagerange); }
			if ((unsigned)min>(unsigned)max) { min=max; error(EM_Invalid_pagerange); }
	...
	while (ii!=p.end())
		if (!output[onr]->pageok(*ii)) {
			error(EM_Page_does_not_exist,tostr(*ii));
			ii=p.erase(ii);
		} else ++ii;
