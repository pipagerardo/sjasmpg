...
int RawSource::_nextline() {
	...
	if( _it==_sl.end() ) {
		...
		if( _unexpectedin.empty() )
			error1( EM_Unexpected_end_of_file );
		else
			error( _unexcurlin, _unexlistcurlin, EM_Unexpected_end_of_file, _unexpectedin );
		return 1;
	}
...
string RawSource::_KillComments() {
	...
	for(;;) {
		...
		for(;;) {
			...
			af=0;
			if (*p==9) res+=' ';
			else if (*p<32) { res+=' '; if (!illchar) { error1(EM_Illegal_character); ++illchar; } }
			else res+=*p;
			++p;
		}
		pos=(int)res.find_last_not_of(' ');
		if( (int)pos!=(int)string::npos ) res.erase(pos+1);
		pos=(int)res.length();
		if (pos && res[pos-1]=='\\') {
			res.erase(pos-1);
			if (_nextline()) return res;
...
void RawSource::_CutLines( string &line ) {
	string res="";
// unsigned pos; // <- En 64 bits error substr
	int pos;
	while ('o') {
...
int RawSource::_ParseLabel( string &line ) {
	...
	if (isdigit(line[0])) {
		int num;
		if (!getval(line,num,10)) { error(EM_Illegal_label_name); return 0; }
		...
		if( needset(line) || needfield(line) || needequ(line) ) {
			error( EM_Numberlabels_only_allowed );
			return 0;
		}
		return 1;
	}
	...
	if (!isalpha(line[0]) && line[0]!='.' && line[0]!='_') { error(EM_Illegal_label_name); return 0; }
	...
	if( needfield(line) ) {
		...
		if (!lnr) { error(EM_Duplicate_labelname,n); return 0; }
		string s=line; int val;
		skipblanks(s);
		if (s.empty()) { val=0; line="0"; }
		else if (!ParseExpression(s,val)) { error(EM_Syntax_error); return 0; }
		...
		string s=line; int val;
		skipblanks(s);
		if (s.empty()) { error(EM_Expression_expected); return 0; }
		if (!ParseExpression(s,val)) { error(EM_Syntax_error); return 0; }
		labtab.setforward(lnr,labelnotfound!=0);
		...
		int lnr=labtab.insert(n);
		if (!lnr) { error(EM_Duplicate_labelname,n); return 1; }
...
void RawSource::_ParseRepeat( string &line ) {
	if (sbcneed(line,'[')) {
		...
		if (p==(int)string::npos) { error( EM_Closing_C_expected ); return; }
		...
		if (rsr->GetSource().empty()) error(EM_Nothing_to_repeat);
		else _source.push_back(new Repeat(count,*rsr->GetSource().begin()));
		_source.back()->Process();
	} else _ExpandMacros(line);
}
...
void RSStructure::_Parse( string &line ) {
	...
	string n, def;
	if( _mustalign ) {
		_mustalign=false; al=_newalign;
	} else al=_defaultalign;
	if( line[0]!=' ' ) {
		if( line[0]=='.' ) line.erase(0,1);
		n=getid(line);
		if( n.empty() ) {
			error1(EM_Illegal_label_name);
		} else {
			if( _global || modlabp.empty() ) def.clear();
			else def=modlabp+'.';
				def+=_s.getname()+'.'+n;
				lnr=labtab.insert(def,false);
				if( lnr )	labtab.setvalue(lnr,0,0);
				else		error(EM_Label_already_exists,def);
		}
	}
		...
		switch( i ) {
			case 1: case 2: case 3: case 4:
				if( !line.empty() ) {
					int d;
					string arg;
					if( !ParseExpression(arg=getargument(line),d) ) error1(EM_Default_value_expected);
			...
			break;
			case 5:
				if (!ParseExpression(line,len)) error1(EM_Size_expected);
				if (labelnotfound) error1(EM_Forward_reference);
				if (comma(line)) {
			...
			case 6:
				...
				if (!ParseExpression(line,al)) {
					al=1; error1( EM_Alignment_value_expected );
				} else {
					if (labelnotfound) error1(EM_Forward_reference);
					if (al<1) { al=1; error1(EM_Alignment_shouldbepositiv); }
				}
				_newalign=al;
			break;
			case 7:
